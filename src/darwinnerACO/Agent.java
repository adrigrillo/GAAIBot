...
public class Agent extends AbstractMultiPlayer {
	...
	// Pesos que se daran a evaluacion y feromona a la hora de escoger las
	// hormigas un camino
	 ...
	 * 
	 * @param stateObs:
	 *            Estado de observacion en el turno actual
	 * @param elapsedTimer:
	 *            Temporizador para la creacion del agente
	 * @param playerID:
	 *            Id del jugador
	 */
	public Agent(StateObservationMulti stateObs, ElapsedCpuTimer elapsedTimer, int playerID) {
		...
		/*
		 * Construccion del grafo inicial. Solo se expande 1 nivel. Estructura:
		 * [0]: estado del juego (accion, vida, score, winner) [1]: hijos [2]:
		 * feromona [3]: evaluacion [4]: probabilidades
		 */
...
//			 Calculamos las probabilidades de decision de camino de las
//			 hormigas


			if (primeraHormiga) {

				// Array de relevancias: indica cuanto de importante es cada
				// estado para la hormiga
				for (int j = 0; j < grafo.length; j += 5) {
					relevancias[j / 5] = pesoEvaluacion * (double) grafo[j + 3] + pesoFeromonas * (double) grafo[j + 2];
					System.out.println(relevancias[j / 5]);
			...
			// En caso de que aun no haya valores en el slot de probabilidades
			// del nodo, escogemos un camino aleatorio
			if (indiceMeta == -1)
	 ...
	 * @param elapsedTimer
	 *            Tiempo en el que la accion acaba, limite de tiempo.
	 * @return
	 */
	public Types.ACTIONS act(StateObservationMulti stateObs, ElapsedCpuTimer elapsedTimer) {
		...
		/*
		 * Evaluacion de caminos de las hormigas (Trasladar al metodo que haga
		 * falta)
		 */
		...
		/*
		 * Potenciar caminos de las hormigas (Trasladar al metodo que haga
		 * falta)
		 */
	}
